...
export default class BarrageLayoutCalculate {
  ...
  constructor({
    ...
    this.virtualTrackAlgorithm = new VirtualTrackAlgorithm(this.br, this);
  }
  ...
  send(barrage: BarrageOptions) {
    ...
    if (barrage.barrageType === 'scroll') {
      ...
      console.log('scrollBarrage', scrollBarrage);
      Utils.Algorithm.insertBarrageByTime(this.scrollBarrageInstances, scrollBarrage);
      console.log('滚动弹幕：', this.scrollBarrageInstances.length);
      Utils.Algorithm.insertBarrageByTime(this.allBarrageInstances, scrollBarrage);
      console.log('所有弹幕：', this.allBarrageInstances.length);
      Utils.Algorithm.insertBarrageByTime(this.canShowScrollBarrageInstances, scrollBarrage);
      console.log('可显示滚动弹幕：', this.canShowScrollBarrageInstances.length);
      ...
      Utils.Algorithm.insertBarrageByTime(this.allBarrageInstances, fixedBarrage);
      ...
      Utils.Algorithm.insertBarrageByTime(this.allBarrageInstances, seniorBarrage);
    }
  }
  ...
  getRenderScrollBarrages(time: number): ScrollBarrage[] {
    // 当前时刻渲染出来的弹幕
    return this.canShowScrollBarrageInstances.filter(barrage => {
      // 这个弹幕在当前时刻是否能渲染
      const currentIsShow =
        time >= barrage.time &&
        time <= barrage.endTime;
      // 如果能渲染的话，计算具体渲染时 left
      if (currentIsShow) {
        barrage.left =
          barrage.originalLeft -
          // 弹幕整体向左移动的总距离，时间 * 速度
          ((time / 1000) * this.br.renderConfig.speed);
      }
      // 返回是否能渲染的标识
