...
async function testMaskGenerator() {
  ...
  const boundingBoxMask = maskGenerator.generateMask(
    segmentationResult,
    dimensions,
  );

  console.log(
    `✓ 生成边界框遮罩: 类型=${boundingBoxMask.type}, 置信度=${boundingBoxMask.confidence.toFixed(3)}`,
  );
  if (boundingBoxMask.type === "BoundingBoxes") {
    ...
    boxes.forEach((box, index) => {
      console.log(
        `  - 框 ${index + 1}: (${box.x}, ${box.y}) ${box.width}x${box.height}`,
      );
    });
  }
  ...
  if (paddedMask.type === "BoundingBoxes") {
    ...
    for (
      let i = 0;
      i < Math.min(originalBoxes.length, paddedBoxes.length);
      i++
    ) {
      const orig = originalBoxes[i];
      const padded = paddedBoxes[i];
      console.log(
        `    框 ${i + 1}: ${orig.width}x${orig.height} → ${padded.width}x${padded.height}`,
      );
    }
  }
  ...
  console.log(
    `✓ 合并遮罩: 类型=${mergedMask.type}, 置信度=${mergedMask.confidence.toFixed(3)}`,
  );

  const mergedStats = maskGenerator.getMaskStats(mergedMask);
  console.log(
    `  - 合并后统计: 大小=${mergedStats.size}, 覆盖=${mergedStats.coverage}`,
  );
  ...
  console.log(
    `  - 优化后统计: 大小=${optimizedStats.size}, 覆盖=${optimizedStats.coverage}`,
  );
  ...
  console.log(
    `✓ 空结果遮罩: 类型=${emptyMask.type}, 置信度=${emptyMask.confidence}`,
  );

  const emptyStats = maskGenerator.getMaskStats(emptyMask);
  console.log(
    `  - 空遮罩统计: 大小=${emptyStats.size}, 覆盖=${emptyStats.coverage}`,
  );

  console.log("\n7. 测试大量分段处理...");
  const largeSegmentationResult = createMockSegmentationResult(5);
  const largeMask = maskGenerator.generateMask(
    largeSegmentationResult,
    dimensions,
  );
  console.log(
    `✓ 大量分段遮罩: 类型=${largeMask.type}, 分段数=${largeSegmentationResult.segments.length}`,
  );

  const largeStats = maskGenerator.getMaskStats(largeMask);
  console.log(
    `  - 大量分段统计: 大小=${largeStats.size}, 覆盖=${largeStats.coverage}`,
  );
  ...
  const pixelMask = maskGenerator.generateMask(
    largeSegmentationResult,
    smallDimensions,
  );
