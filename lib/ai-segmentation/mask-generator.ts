...
import type { IMaskGenerator } from "./interfaces";
import type {
  SegmentationResult,
  MaskData,
  Dimensions,
...
export class MaskGenerator implements IMaskGenerator {
  ...
  generateMask(
    ...
    // 根据分段数量选择最佳遮罩类型
    const maskType = this.determineMaskType(
  ...
  private generateBoundingBoxMask(
    ...
    segmentationResult.segments.forEach((segment) => {
      ...
      const paddedBox = this.expandBoundingBox(
        segment.boundingBox,
        padding,
        dimensions,
      );
  ...
  private expandBoundingBox(
    ...
    return {
      ...
      width: Math.min(
        dimensions.width - Math.max(0, box.x - padding.left),
        box.width + padding.left + padding.right,
      ),
      height: Math.min(
        dimensions.height - Math.max(0, box.y - padding.top),
   ...
   * 应用边界框填充
   */
  private applyBoundingBoxPadding(
    mask: MaskData,
    padding: PaddingConfig,
  ): MaskData {
   ...
   * 应用像素遮罩填充
   */
  private applyPixelMaskPadding(
    mask: MaskData,
    _padding: PaddingConfig,
  ): MaskData {
