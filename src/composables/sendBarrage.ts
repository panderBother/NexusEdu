...
import type {
  BarrageImage,
  FixedBarrageOptions,
  ScrollBarrageOptions,
  SeniorBarrageOptions,
  SeniorBarrageConfig,
} from "../../lib/index.ts";
...
export default function useSendBarrage(
  ...
  // 弹幕模式
  const barrageModes = ref<
    { label: string; value: "scroll" | "top" | "bottom" }[]
  >([
  ...
  const sendBarrage = () => {
    ...
    barrage.prior = true; // 让刚发送的弹幕优先调度渲染，避免被旧弹幕遮挡
    ...
    const barrage: SeniorBarrageOptions = {
      ...
      seniorBarrageConfig: {
        ...
        endLocation: {
          ...
          offsetY: -(
            lanternHeight +
            text.length * (currentFontsize.value * 1.2)
          ),
        },
        totalDuration: 20000,
        delay: 0,
