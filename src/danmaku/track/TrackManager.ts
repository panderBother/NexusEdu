...
import type { Track, DanmakuItem, TrackType } from '../types'
...
export class TrackManager implements ITrackManager {
   ...
   * 根据屏幕高度和弹幕大小划分虚拟轨道
   */
  initialize(canvasHeight: number, trackHeight: number, trackGap: number, canvasWidth: number = 1920): void {
    ...
    // 初始化底部轨道
    this.bottomTracks = []
    const bottomStartY = canvasHeight - (bottomTrackCount * (trackHeight + trackGap))
  ...
  getAvailableTrackCount(): number {
    ...
    const allTracks = [...this.scrollTracks, ...this.topTracks, ...this.bottomTracks]
  ...
  getTrackCount(): number {
    return this.scrollTracks.length + this.topTracks.length + this.bottomTracks.length
  }
